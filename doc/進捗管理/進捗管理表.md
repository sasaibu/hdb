...
| | 日付・時刻ピッカー | TBD | 2025-07-24 | 2025-07-25 | **仕様書必須機能未実装** | 0% | 3 | 入力フォーム | DateTimePicker | **高** | **【問題】** 仕様書優先度A（必須）だが未実装。現在は`new Date().toISOString()`で現在時刻自動設定のみ。ユーザーが日付・時刻を選択できない状態。**【必要作業】** react-native-date-pickerライブラリインストール、NewVitalInputDialogに日付・時刻選択UI追加、バリデーション実装 |
...
| 35 | 新アプリ→バイタルAWS | API | 通知設定取得 | 通知設定の読み込み | ❌ 未実装 | NotificationSettingsScreen.tsx | 米津 | 通知設定画面表示時 | AsyncStorage直接使用→apiClient.getNotificationSettings()への変更 |
| 36 | 新アプリ→バイタルAWS | API | 通知設定更新 | 通知設定の保存 | ❌ 未実装 | NotificationSettingsScreen.tsx | 米津 | 通知設定変更時 | AsyncStorage直接使用→apiClient.updateNotificationSettings()への変更 |
| 37 | 新アプリ→バイタルAWS | API | 連携サービス設定取得・更新 | HealthKit/ヘルスコネクト設定管理 | ❌ 未実装 | LinkedServicesSettingsScreen.tsx | 米津 | 連携設定変更時 | AsyncStorage直接使用→apiClient経由への変更 |
...
### ライブラリ照合サマリー（2025-07-24作成）
- ✅ 一致: 10個 (31.3%)
- ⚠️ バージョン差異: 3個 (9.4%)
- ❌ 未インストール: 19個 (59.4%)
- **合計**: 32個（使用ライブラリ一覧記載）
- **実装率**: 40.6% (13/32個が何らかの形で実装済み)
...
| **入力・ユーティリティ** |  |  |  |  |  |  |
| react-native-date-picker | ^4.4.0 | 未インストール | ❌ 未実装 | 中 | 2025-08-15 | 日付選択 |
| moment | ^2.30.1 | 未インストール | ❌ 未実装 | 中 | 2025-08-15 | 日時処理 |
| **開発** |  |  |  |  |  |  |
