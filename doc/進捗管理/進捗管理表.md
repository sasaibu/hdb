...
| 6 | メニュー | ユーザ権限をバイタルAWSから取得し、権限がないメニューについては、バーガーメニューで非表示とする | 完了 | |　getUserPermissions() メソッドでユーザー権限を返す、ユーザー別の権限設定。testuser:全権限あり limiteduser/demouser*制限付き権限(一部メニュー非表示) |
| 7 | ナビゲーション | ユーザ権限をバイタルAWSから取得し、権限がないメニューについては、ナビゲーションに設置しない | 完了 | 山崎 | ユーザーがログインすると、自動的にバイタルAWS(プロトの段階ではモック)から権限情報を取得|
| 8 | メニュー | データ移行完了時に、データ移行メニューを非活性とする | 完了 | 山崎| データ移行完了後はメニューに表示されたままグレーアウトされ、非活性状態となります。クリックするとデータ移行はすでに完了してありますと表示|
...
| 12 | 規約 | 規約に変更があった場合、アプリ起動時に規約画面（同意確認あり）を表示させたい | 完了 | 山崎 | |
...
| 16 | マイページ | ニックネームの表示・非表示を選択可能とする | 完了 |山崎 | |
