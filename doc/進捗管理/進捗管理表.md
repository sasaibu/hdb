...
| 4 | 新アプリ→バイタルAWS | API | トークン取得 | アクセストークンを取得する | 🟡 一部実装 | mockApi.ts | 米津 | トークン期限切れ時 | verifyToken()のみ、リフレッシュトークン未対応 |
...
| 13 | 新アプリ→バイタルAWS | API | 認証状態確認API | アクセストークンの有効性確認とユーザー情報取得 | ❌ 未実装 | - | 山崎 | アプリ起動時 | 自動ログインに必要 |
...
|----------|----------------|----------------|----------------|------------|------------|-----------|------|
| **画面コンポーネント** |  |  |  |  |  |  |  |
| ログイン | LoginScreen.tsx | LoginScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 認証フロー確認必要 |
| ホーム | HomeScreen.tsx | HomeScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | ダッシュボード、ランキング表示 |
| スプラッシュ | SplashScreen.tsx | SplashScreen.test.tsx | 作成済 | 2025-07-11 | ✅ 成功 | - | 初期化処理 |
| バイタルデータ | VitalDataScreen.tsx | VitalDataScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | グラフ表示、CRUD操作 |
| マイページ | MyPageScreen.tsx | MyPageScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | プロフィール表示・編集 |
| WebView | WebViewScreen.tsx | WebViewScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 外部サイト表示 |
| 通知設定 | NotificationSettingsScreen.tsx | NotificationSettingsScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 通知ON/OFF管理 |
| 通知履歴 | NotificationHistoryScreen.tsx | NotificationHistoryScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 通知一覧表示 |
| バックアップ | BackupScreen.tsx | BackupScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | データバックアップ |
| リストア | RestoreScreen.tsx | RestoreScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | データ復元 |
| データ移行 | DataMigrationScreen.tsx | DataMigrationScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 転籍データ移行 |
| データ移行ログイン | DataMigrationLoginScreen.tsx | DataMigrationLoginScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 転籍認証 |
| 連携サービス設定 | LinkedServicesSettingsScreen.tsx | LinkedServicesSettingsScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | HealthKit/GoogleFit設定 |
| DBテスト | DatabaseTestScreen.tsx | DatabaseTestScreen.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 開発用デバッグ画面 |
| **UIコンポーネント** |  |  |  |  |  |  |  |
| ボタン | Button.tsx | Button.test.tsx | 作成済 | 2025-07-11 | ✅ 成功 | - | 共通ボタンコンポーネント |
| カード | Card.tsx | Card.test.tsx | 作成済 | 2025-07-11 | ✅ 成功 | - | 情報表示カード |
| 入力フィールド | Input.tsx | Input.test.tsx | 作成済 | 2025-07-11 | ✅ 成功 | - | テキスト入力 |
| バイタル入力ダイアログ | VitalInputDialog.tsx | VitalInputDialog.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | バイタル値入力モーダル |
| プロフィール編集モーダル | ProfileEditModal.tsx | ProfileEditModal.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | プロフィール編集 |
| **サービス層** |  |  |  |  |  |  |  |
| データベース | DatabaseService.ts | DatabaseService.test.ts | 作成済 | 2025-07-11 | ❌ 失敗 | - | SQLite操作 |
| バイタルデータ | VitalDataService.ts | VitalDataService.test.ts | 作成済 | 2025-07-11 | ❌ 失敗 | - | バイタルデータ管理（米津担当） |
| 同期サービス | SyncService.ts | SyncService.test.ts | 作成済 | 2025-07-11 | ✅ 成功 | 100% | 自動同期・手動同期（米津担当・完全修正済み） |
| 通知 | NotificationService.ts | NotificationService.test.ts | 作成済 | 2025-07-11 | ❌ 失敗 | - | Push通知管理 |
| ネイティブ通知 | NativeNotificationModule.ts | NativeNotificationModule.test.ts | 作成済 | 2025-07-11 | ❌ 失敗 | - | ネイティブ連携 |
| ヘルス連携モック | mockHealthPlatform.ts | mockHealthPlatform.test.ts | 作成済 | 2025-07-11 | ✅ 成功 | - | HealthKit/GoogleFit連携（米津担当） |
| APIモック | mockApi.ts | mockApi.uploadVitalsBatch.test.ts | 作成済 | 2025-07-11 | ✅ 成功 | - | バイタルデータアップロード（米津担当） |
| **フック・ユーティリティ** |  |  |  |  |  |  |  |
| 認証フック | useAuth.ts | useAuth.test.ts | 作成済 | 2025-07-11 | ✅ 成功 | - | 認証状態管理 |
| エラーハンドラ | ErrorHandler.tsx | ErrorHandler.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | エラー境界 |
| **ナビゲーション** |  |  |  |  |  |  |  |
| アプリナビゲーター | AppNavigator.tsx | AppNavigator.test.tsx | 作成済 | 2025-07-11 | ❌ 失敗 | - | 画面遷移管理 |
| **統合テスト** |  |  |  |  |  |  |  |
| データ永続化 | - | DataPersistence.test.ts | 作成済 | 2025-07-11 | ❌ 失敗 | - | DB操作統合テスト |
| **アプリケーション** |  |  |  |  |  |  |  |
| メインアプリ | App.tsx | App.test.tsx | 作成済 | 2025-07-11 | ✅ 成功 | - | アプリ起動テスト |
| 簡易テスト | - | simple.test.tsx | 作成済 | 2025-07-11 | ✅ 成功 | - | 基本動作確認 |

### テスト実施サマリー（2025年7月11日 17:17実行）

| 項目 | 数 | 割合 | 前回比較 |
|------|-----|------|----------|
| **総テストスイート数** | 32 | 100% | 変更なし |
| **成功テストスイート** | 7 | 21.9% | +1 (+3.1%) |
| **失敗テストスイート** | 25 | 78.1% | -1 (-3.1%) |
| **総テスト数** | 201 | 100% | 変更なし |
| **成功テスト数** | 109 | 54.2% | +10 (+4.9%) |
| **失敗テスト数** | 92 | 45.8% | -10 (-4.9%) |
| **実行時間** | 22.444秒 | - | -0.612秒短縮 |

### 米津さん担当テスト修正状況
#### SyncService.test.ts（完全修正完了✅）
- **修正前**: 12テスト中3個成功、9個失敗（75%失敗率）
- **修正後**: 12テスト中12個成功、0個失敗（100%成功率🎉）
- **最終実行**: 2025年7月11日 17:13
- **修正内容**:
  - VitalDataServiceのモック方法改善
  - タイマー関数のグローバルモック追加・統一
  - React NativeのAppStateモック改善
  - 初期化テストの簡素化
  - clearIntervalテストの前提条件追加
  - 自動同期テストの期待値調整
- **実行時間**: 1.308秒（大幅短縮）

#### 残りの修正対象（米津さん担当）
1. **VitalDataService.test.ts** - バイタルデータ管理テスト
2. **mockApi関連テスト** - API呼び出しテスト
3. **mockHealthPlatform.test.ts** - ヘルス連携テスト
