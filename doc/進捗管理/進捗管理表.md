...
| **🟡 高優先度修正作業** | **Healthデータ登録API修正** | 完了 | 2025-07-18 | 2025-07-18 | 完了 | 100% | 1.5 | 血圧データ拡張 | API仕様変更 | 中 | mockApi.ts uploadVitalsBatch() - 測定項目コード変換、value1/value2/value3構造、日中データ対応、削除フラグ対応。サーバー側API仕様変更によりアプリ側も新形式でのデータ送信が必要となったため対応。画面変更なし、内部処理のみ修正 ✅ |
| | **マイデータ登録API修正** | 完了 | 2025-07-18 | 2025-07-18 | 完了 | 100% | 1 | Healthデータ登録API | API仕様変更 | 中 | mockApi.ts updateProfile() - ニックネーム表示フラグ、目標設定・達成情報、プッシュ通知設定、応援コメント・目標プッシュ通知レスポンス。新IF仕様書No.9に基づきmypage/goal/goal_achievement/push構造に対応、support_comment/goal_pushレスポンス実装。既存コード互換性のためupdateProfileLegacy()も追加 ✅ |
| | **新ER図対応修正** | 完了 | 2025-07-18 | 2025-07-18 | 完了 | 100% | 1 | マイデータ登録API | 画面・DB構造変更 | 高 | MyPageScreen.tsx, ProfileEditModal.tsx - アイコン機能削除、新ER図my_dataテーブル構造対応、S3キー管理削除、ローカル画像パス削除。新ER図でアイコン機能が廃止されたため、マイページからアイコン表示・編集機能を完全削除。ニックネームのみの表示に変更 ✅ |
| | **デバイス情報登録API修正** | 完了 | 2025-07-18 | 2025-07-18 | 完了 | 100% | 1 | 新ER図対応修正 | API仕様変更 | 中 | DeviceInfoService.ts新規作成、SplashScreen.tsx修正、mockApi.ts registerDevice()修正 - 新ER図device_infoテーブル対応（デバイスID、OS、バージョン、デバイストークン最終使用日時270日期限管理）、おすすめ目標配列、移行データ配列、利用権限配列、1日1回実行制御完全実装 ✅ |
