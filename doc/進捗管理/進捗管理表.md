...
| | iOS開発環境（Xcode） | TBD | TBD | TBD | 実装対象外 | 対象外 | 2 | - | iOS開発環境なし、Android開発のみ対応 |
...
| **データベース** |  |  |  |  |  |  |
| react-native-sqlite-storage | ^6.0.1 | 未インストール | ❌ 未実装 | 中 | 2025-08-15 | react-native-sqlite-2使用中 |
| react-native-sqlite-2 | ^3.7.0 | ^3.6.2 | ⚠️ バージョン差異 | 低 | 2025-08-30 | 古いバージョン使用中 |
| **API通信** |  |  |  |  |  |  |
| axios | ^1.7.2 | 未インストール | ❌ 未実装 | プロトタイプ不要 | - | プロトタイプ段階ではモック実装で十分 |
| **ヘルスデータ** |  |  |  |  |  |  |
| react-native-health | ^1.19.0 | 未インストール | ❌ 未実装 | プロトタイプ不要 | - | プロトタイプ段階では不要、モック実装済み |
| react-native-health-connect | ^1.3.0 | 未インストール | ❌ 未実装 | プロトタイプ不要 | - | プロトタイプ段階では不要、モック実装済み |
| @react-native-sensors/pedometer | ^7.2.0 | 未インストール | ❌ 未実装 | 中 | 2025-08-15 | **プロトタイプでは不要**。mockHealthPlatform.tsで歩数データを含む6種類のバイタルデータ（歩数、体重、体温、血圧、心拍数）のモック実装完了。本番ではiOS以外のAndroid端末で直接歩数計にアクセスする際に必要（Health Connectが使えない古い端末向け） |
| **通知・設定** |  |  |  |  |  |  |
| @react-native-firebase/app | ^19.2.2 | 未インストール | ❌ 未実装 | プロトタイプ不要 | - | インフラのため、プロトタイプ段階ではモック実装で十分 |
| @react-native-firebase/messaging | ^19.2.2 | 未インストール | ❌ 未実装 | プロトタイプ不要 | - | インフラのため、プロトタイプ段階ではモック実装で十分 |
| @react-native-firebase/remote-config | ^19.2.2 | 未インストール | ❌ 未実装 | プロトタイプ不要 | - | インフラのため、プロトタイプ段階ではモック実装で十分 |
| @react-native-community/netinfo | ^11.3.1 | 未インストール | ❌ 未実装 | 中 | 2025-08-15 | **プロトタイプでは不要**。現在ErrorHandler.tsxでネットワークエラーを「ネットワークに接続できません」として処理。SyncServiceで自動同期は実装済みだが、オフライン時の待機処理は未実装。本番では同期前のネットワーク状態確認、オフライン時のローカル保存、オンライン復帰時の自動同期に必要 |
| **UI部品** |  |  |  |  |  |  |
| react-native-vector-icons | ^10.0.3 | ^10.0.3 | ✅ 実装済み | - | - | アイコン表示実装完了、HomeScreenで使用中 |
| react-native-chart-kit | ^6.12.0 | 未インストール | ❌ 未実装 | ペンディング | - | 外部ライブラリ導入困難のため、React Native標準のView/StyleSheetコンポーネントでグラフ実装済み |
| react-native-modal | ^13.0.1 | 未インストール | ❌ 未実装 | 低 | 2025-09-01 | **プロトタイプでは不要**。VitalInputDialog、NewVitalInputDialog、ProfileEditModal等、7つの画面でReact NativeデフォルトのModalコンポーネントを使用中。現在の実装で問題なし。react-native-modalはスワイプで閉じる、背景アニメーション等の追加機能を提供するが、現時点では不要 |
| react-native-picker-select | ^9.1.3 | 未インストール | ❌ 未実装 | 低 | 2025-09-01 | **プロトタイプでは不要**。TimingInputScreenで時間選択（時・分）を独自のScrollView+TouchableOpacityで実装済み。選択可能な値を表示し、タップで選択、選択中の値は背景色とフォントで強調表示。ネイティブのピッカーより視覚的にわかりやすいUI実現 |
| **外部連携** |  |  |  |  |  |  |
| @react-native-google-signin/google-signin | ^10.1.0 | 未インストール | ❌ 未実装 | 低 | 2025-09-01 | Google認証 |
| @react-native-community/icloud-storage | ^1.1.0 | 未インストール | ❌ 未実装 | 低 | 2025-09-01 | iCloud連携 |
| @react-native-google-drive/google-drive | ^1.1.0 | 未インストール | ❌ 未実装 | 低 | 2025-09-01 | Google Drive連携 |
| **入力・ユーティリティ** |  |  |  |  |  |  |
| react-native-date-picker | ^4.4.0 | 未インストール | ❌ 未実装 | 高 | 2025-07-25 | **2025-07-25インストール試行も動作エラー、アンインストール済み。TextInput方式で代替実装中（50%）** |
| moment | ^2.30.1 | 未インストール | ❌ 未実装 | 中 | 2025-08-15 | **日付・時刻ピッカーをTextInput方式にしたため、関連ライブラリも導入見送り** |
| **開発** |  |  |  |  |  |  |
| flipper-plugin-react-native-performance | ^0.1.0 | 未インストール | ❌ 未実装 | 低 | 2025-09-01 | パフォーマンス監視 |

### ライブラリ実装計画

#### Phase 1: 高優先度ライブラリ（必須機能）- 2025年8月1日
- **現在は該当なし** - プロトタイプ段階では以下はモック実装で十分
  - axios（API通信）
  - react-native-health（iOS HealthKit連携）
  - react-native-health-connect（Android Health Connect連携）
  - @react-native-firebase/app（Firebase Core）
  - @react-native-firebase/messaging（Push通知）

#### Phase 2: 中優先度ライブラリ（機能拡張）- 2025年8月15日
- ~~**react-native-vector-icons**~~ - ✅ 実装済み
- ~~**react-native-date-picker**~~ - ❌ TextInput方式で代替実装中
- ~~**moment**~~ - ❌ 導入見送り（日付・時刻ピッカー関連）
- ~~**@react-native-sensors/pedometer**~~ - ❌ mockHealthPlatformで代替済み
- ~~**@react-native-community/netinfo**~~ - ❌ エラーハンドリングで対応済み

#### ペンディング
- **react-native-chart-kit** - 外部ライブラリ導入困難のため、React Native標準コンポーネントで代替実装済み

#### Phase 3: 低優先度ライブラリ（オプション）- 2025年9月1日
- ~~**react-native-modal**~~ - ❌ React NativeデフォルトのModalで十分
- ~~**react-native-picker-select**~~ - ❌ 独自UI実装済み
- **@react-native-google-signin/google-signin** - Google認証
- **@react-native-community/icloud-storage** - iCloud連携
- **@react-native-google-drive/google-drive** - Google Drive連携
- **flipper-plugin-react-native-performance** - パフォーマンス監視
...
| フェーズ | 対象ライブラリ数 | 完了数 | 進捗率 | 予定完了日 | 状況 |
|----------|-----------------|--------|--------|------------|------|
| Phase 1 | 5個 | 0個 | 0% | 2025-08-01 | プロトタイプではモック実装で十分 |
| Phase 2 | 5個 | 1個 | 20% | 2025-08-15 | 一部実装済み |
| Phase 3 | 6個 | 0個 | 0% | 2025-09-01 | 未着手 |
| バージョン差異対応 | 4個 | 0個 | 0% | 2025-08-30 | 未着手 |
| **合計** | **20個** | **1個** | **5%** | **2025-09-01** | **実装開始** |
