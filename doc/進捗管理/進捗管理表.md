...
|-----|----------|----------|---------------------|--------|------|
| 1 | 目標設定 | 目標入力時は、パルスサーベイ、健康診断が見えるようにしたい。30日達成後の２回目の目標設定時、ナビゲーションのホームしか活性化されていないので、上記２つが見えない。⇒解決方法２案：①初回起動時の説明画面を２回目以降も表示する、②目標確定まで、ナビゲーションのパルスサーベイ、健康診断も活性とする | 未完了 | | |
| 2 | 目標設定 | 30日達成後の祝メッセージのダイアログを閉じた先は、30日目のDone画面とする | 未完了 | | |
| 3 | 目標設定 | 30日達成後の次に目標を設定するタイミングは持ち帰り中 | 未完了 | | |
| 4 | バイタル管理 | 脈拍だけでなく、心拍も追加・血圧に最高、最低、脈拍・血圧とは別で（体重などと同じラインで）心拍（ドロップ済み） | - | | No.23に変更 |
...
| 9 | 非機能要件 | 多言語対応は初期リリースでは不要と整理 | - | | 対応不要 |
...
| 15 | マイページ | マイページでのアイコン選択はドロップ（＝アイコンなし） | - | | アイコンなし |
| 16 | マイページ | ニックネームの表示・非表示を選択可能とする | 未完了 | | |
| 17 | その他 | 転籍データ移行のログイン画面後に、転籍データ移行に関する規約画面を表示する⇒アプリ側での影響なし | - | | アプリ側影響なし |
| 18 | その他 | アクティブ率を取りたい⇒デバイス登録APIにてバイタルAWSで集計出す⇒アプリ側での影響なし | - | | アプリ側影響なし |
...
| 32 | プロトタイプ | 設計、製造工程の早期タイミングでプロトタイプを構築し、アプリ昨日の確認＆ＦＢの反映できるスケジュールを立てる | 未完了 | | |
...
| 35 | バックアップ | アプリ内DBエンジンが異なることでバイタルAWSにバックアップを行うときkのファイルフォーマットがAndroidとiOSで差異が発生する可能性あり。iOS側で通常登録でなくライブラリを利用する想定であるがフィジビリティ取る必要がある | 未完了 | | |
...
|------|------|------|------|
| **総課題数** | 35件 | 100% | |
| **要実装課題** | 30件 | 85.7% | 実装が必要な課題 |
| **対応不要** | 5件 | 14.3% | ドロップ・影響なし等 |
| **実装完了** | 0件 | 0% | プロトタイプで実装済み |
| **未実装** | 30件 | 100% | プロトタイプ未実装 |
